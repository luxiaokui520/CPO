...
class TestParseStr(unittest.TestCase):
    ...
    def test_custom_function(self):
        str_1 = 'f(x)+1.1'
        self.assertEqual(
            calculate(number_list=parse_str(str_1), custom_function=lambda x: x + 1, custom_parameter={'x': 1.1}), 3.2)
        self.assertEqual(
            calculate(number_list=parse_str(str_1), custom_function=lambda x: x + 1, custom_parameter={'x': 2}), 4.1)
    ...
    def test_mixed_str(self):
        ...
        str_4 = '1+f(x,y)+z'
        self.assertEqual(
            calculate(number_list=parse_str(str_1), custom_function=lambda x: x + 1, custom_parameter={'x': 1}), 22)
        self.assertEqual(
            calculate(number_list=parse_str(str_2), custom_function=lambda x: x * 1, custom_parameter={'x': 1}), 26)
        self.assertEqual(
            calculate(number_list=parse_str(str_3), custom_function=lambda x: x ** 2, custom_parameter={'x': 1}), 2)
        self.assertEqual(calculate(number_list=parse_str(str_4), custom_function=lambda x, y: x * y,
                                   custom_parameter={'x': 2, 'y': 5, 'z': 8}), 19)
